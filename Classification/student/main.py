...
def generate_multi_class_feature_matrix(df, word_dict):
    number_of_reviews = df.shape[0]
    number_of_words = len(word_dict)
    feature_matrix = np.zeros((number_of_reviews, number_of_words))
    row_number = 0
...
def get_average_non_zeroes(X_train):
    # function that return the average non_zeroes per row
    average_zeroes = 0
    for i in X_train:
        non_zero_counter = 0
        for j in i:
...
def one_vs_all_implementation(X_train, Y_train, X_test, Y_test):
    ...
    #Y_test_third = Y_test
    #Y_train_third[Y_train_third == -1] = 1
    #Y_test_third[Y_test_third == -1] = 1
...
def main():
    ...
    #3c
    #print("Average number of nonzero features:", get_average_zeroes(X_train))
